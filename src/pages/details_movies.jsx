...
const CastCard = memo(({ person }) => (
  <View
    ...
    accessibilityLabel={`${person.name}, ${person.character}`}
  >
    {person.profile_path ? (
      <Image
...
const MovieDetail = ({ route, navigation }) => {
  ...
  const renderCast = useCallback(({ item }) => <CastCard person={item} />, []);

  const renderTrailer = useCallback(
    ({ item }) => <TrailerCard video={item} />,
    [],
  );
  ...
  const backdrop = movieData?.backdrop_path
    ? `${IMG_BASE}/w780${movieData.backdrop_path}`
    : routeCover;
  const poster = movieData?.poster_path
    ? `${IMG_BASE}/w500${movieData.poster_path}`
    : routeCover;
  ...
  const releaseYear =
    releaseDate !== "TBA" ? new Date(releaseDate).getFullYear() : "TBA";
  const status = movieData?.status || "Unknown";
  const voteAvg = movieData?.vote_average
  ...
  const recommendations =
    movieData?.recommendations?.results?.slice(0, 15) || [];
  ...
  const allReviews = [
    ...dbReviews.map((r) => ({
      id: r.id,
      name:
        r.profiles?.use_display_name && r.profiles?.display_name
    ...
    ...tmdbReviews.map((r) => ({
      ...
      rating: r.author_details?.rating
        ? Math.ceil(r.author_details.rating / 2)
        : null,
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.statsSection}>
          <StatsPill
            label="Rating"
            count={voteAvg ? `${voteAvg}%` : "N/A"}
            color={ACCENT}
          />
          <StatsPill
            label="Votes"
            count={voteCount.toLocaleString()}
            color={ACCENT2}
          />
        ...
        {(director || producers.length > 0) && (
          <GlassCard style={styles.blurCard}>
            ...
            <View style={styles.companyList}>
              {director ? (
                <CompanyRow name={director.name} role="Director" />
              ) : null}
              {writers.map((w, i) => (
                <CompanyRow key={`w-${i}`} name={w.name} role={w.job} />
              ))}
              {producers.map((p, i) => (
                <CompanyRow
                  key={`p-${i}`}
